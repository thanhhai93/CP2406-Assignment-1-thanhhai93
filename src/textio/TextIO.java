...
// NOTE: improved detection of when previous stream is not standard output
...
public class TextIO {
    ...
    public static void writeStandardOutput() {
        if (writingStandardOutput || outputFileName == null)
            return;
        try {
            out.close();
        }
    ...
    private static void skipWhitespace() {
        ...
        while (ch != EOF && Character.isWhitespace(ch)) {
            readChar();
            if (ch == '\n' && readingStandardInput && (writingStandardOutput || outputFileName == null)) {
                out.print("? ");
                out.flush();
            }
            ch = lookChar();
    ...
    private static void errorMessage(String message, String expecting) {  // Report error on input.
        if (readingStandardInput && (writingStandardOutput || outputFileName == null)) {
